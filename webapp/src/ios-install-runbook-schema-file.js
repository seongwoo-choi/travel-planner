...
function writeAtomic(filePath, body) {
  ...
  try {
    ...
    console.error(`ios-install-runbook-schema=${resolvedPath}`);
  } catch (error) {
    rmSync(tempPath, { force: true });
    console.error(`ios-install-runbook-schema=failed (${error.message})`);
    process.exit(1);
  }
}

const sourcePath = path.join(srcDir, "ios-install-runbook.schema.json");
