...
function parseArgs(args) {
  return args.reduce((options, arg) => {
    ...
    output: "",
    outputEnv: "",
    outputDefault: "reports/ios-install-next.schema.json",
  });
}
...
const schemaPath = path.join(webappDir, "src", "ios-install-next.schema.json");

mkdirSync(path.dirname(outputPath), { recursive: true });
copyFileSync(schemaPath, outputPath);
console.log(`ios-install-next-schema=${outputPath}`);
