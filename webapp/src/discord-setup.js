...
function publicBaseUrlStatus() {
  ...
  if (!rawUrl) {
    return [
      ...
      "  Discord commands still work from any connected device while the bot process is running.",
  ...
  if (host === "localhost" || host === "127.0.0.1" || host === "::1") {
    return [
      ...
      "  This opens on the Mac itself, not from another device. Use the Mac LAN IP, a tunnel/VPN URL, or a deployed URL.",
    ];
  }
  ...
  if (/^(10|192\.168|172\.(1[6-9]|2\d|3[0-1]))\./.test(host)) {
    return [
      ...
      "  This works from other devices only on the same network, unless you add VPN or tunneling.",
    ];
  }

  return [
    `Web detail links: external-looking URL (${rawUrl})`,
    "  If the URL is reachable from another device, Discord web buttons should open outside the home Wi-Fi too.",
  ];
}
...
function main() {
  ...
  console.log(`- Admin-only commands: ${adminUserIds.length > 0 ? `${adminUserIds.length} admin user(s)` : "off"}`);
  console.log("- Use /whoami in Discord to find user ID and server ID.");
  console.log("- Use the settings button to get a ready-to-copy Discord .env snippet.");
  console.log("- /start, /whoami, /policy, and /recover still answer as setup/ID recovery commands when access settings are not ready.");
  console.log("- Blocked Discord responses include recovery buttons: 외부 사용, 설정, 내 ID, 정책.");

  console.log("\nMobile mode:");
  console.log("- Mobile Discord commands do not require the Mac and device to share Wi-Fi.");
  ...
  console.log("- Run npm run health:api to check the running health endpoint.");
  console.log("- Run npm run health:api:gate to make a failed health check exit non-zero.");
  console.log("- Set TRAVEL_HEALTH_EVIDENCE=1 when you need target/status/timing diagnostic output.");
  ...
  console.log("/start, /home, /mobile, /offline, /recover, /whoami, /policy, /doctor, /ops, /denied, /status, /dashboard, /now, /nextaction, /help, /guide");

  console.log("\nRun:");
  console.log("npm run bot:doctor # 로컬 env/DB/launchd 설정 점검");
  console.log("npm run health:api # 실행 중인 health API 점검");
  console.log("npm run health:api:gate # health API 실패 시 non-zero 종료");
