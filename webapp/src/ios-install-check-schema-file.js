...
function outputPath(options) {
  return options.output
    || process.env.TRAVEL_IOS_INSTALL_CHECK_SCHEMA_PATH
...
async function main() {
  ...
  if (!targetPath) {
    throw new Error("missing --output, TRAVEL_IOS_INSTALL_CHECK_SCHEMA_PATH, or --output-default");
  }

  const sourcePath = path.join(srcDir, "ios-install-check.schema.json");
  ...
  console.log(`ios-install-check-schema=${resolvedTargetPath}`);
}

main().catch((error) => {
  console.error("ios-install-check-schema=failed");
