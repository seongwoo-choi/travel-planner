...
import { buildQualityGateCiExamples, buildQualityGateCommands } from "./quality-todo-core.js";
...
export async function getPlanQualitySummary(dbPath = DEFAULT_DB_PATH, options = {}) {
  ...
  const summary = plans.reduce(
    (result, plan) => {
      const meta = latestRevisionMeta(plan);
      if (Number(meta.qualityCheckCount || 0) > 0) {
        result.qualityAudited += 1;
      } else {
        result.qualityUnaudited += 1;
      }
      if (Number(meta.qualityWarningCount || 0) > 0) result.quality += 1;
      if (Number(meta.qualityActionPriority || 0) >= 80) result.qualityUrgent += 1;
      if (Number(meta.qualityCheckCount || 0) > 0 && Number(meta.qualityWarningCount || 0) === 0) result.qualityOk += 1;
      if (Number(meta.qualityWarningDelta || 0) > 0) result.qualityRegression += 1;
      if (Number(meta.qualityWarningDelta || 0) < 0) result.qualityImproved += 1;
      (meta.qualityWarnings || []).forEach((label) => {
  ...
  summary.qualityAction = summary.quality + summary.qualityUnaudited;
  summary.qualityOkRate = summary.qualityAudited > 0 ? Math.round((summary.qualityOk / summary.qualityAudited) * 100) : null;
  if (summary.qualityUrgent > 0) {
    summary.qualityNextFilter = "quality-urgent";
    summary.qualityNextLabel = "긴급 후보";
    summary.qualityNextReason = "우선도 80 이상 긴급 후보를 먼저 보강하세요.";
  } else if (summary.qualityRegression > 0) {
    summary.qualityNextFilter = "quality-regression";
    summary.qualityNextLabel = "품질 악화";
    summary.qualityNextReason = "직전 버전보다 확인 항목이 늘어난 플랜을 먼저 보강하세요.";
  } else if (summary.quality > 0) {
    summary.qualityNextFilter = "quality";
    summary.qualityNextLabel = "품질 확인";
    summary.qualityNextReason = "품질 확인 항목이 남은 플랜을 보강하세요.";
  } else if (summary.qualityUnaudited > 0) {
    summary.qualityNextFilter = "quality-unaudited";
    summary.qualityNextLabel = "품질 미점검";
    summary.qualityNextReason = "자동 품질 점검이 없는 플랜에 점검을 생성하세요.";
  } else {
    summary.qualityNextFilter = "";
    summary.qualityNextLabel = "";
    summary.qualityNextReason = "현재 품질 보강이 필요한 플랜은 없습니다.";
  }
  summary.qualityNextApiPath = summary.qualityNextFilter
    ? `/api/plans?filter=${encodeURIComponent(summary.qualityNextFilter)}&limit=10`
    : "";
  summary.qualityNextTodoPath = summary.qualityNextFilter
    ? "/api/plans/quality-todo?next=true&limit=10"
    : "";
  summary.qualityNextTodoTextPath = summary.qualityNextFilter
    ? "/api/plans/quality-todo.txt?next=true&limit=10"
    : "";
  summary.qualityGatesPath = "/api/plans/quality-gates";
  summary.qualityGatesTextPath = "/api/plans/quality-gates.txt";
  summary.qualityGatesCsvPath = "/api/plans/quality-gates.csv";
  summary.qualityGatesCsvGatePath = "/api/plans/quality-gates.csv?failOnFailed=true";
  summary.qualityGatesReportPath = "/api/plans/quality-gates.report.md";
  summary.qualityGatesReportGatePath = "/api/plans/quality-gates.report.md?failOnFailed=true";
  summary.qualityGatesMetricsPath = "/api/plans/quality-gates.metrics";
  summary.qualityGatesMetricsGatePath = "/api/plans/quality-gates.metrics?failOnFailed=true";
  summary.qualityGatesEventsPath = "/api/plans/quality-gates.events.ndjson";
  summary.qualityGatesEventsGatePath = "/api/plans/quality-gates.events.ndjson?failOnFailed=true";
  summary.qualityGatesAlertPath = "/api/plans/quality-gates.alert.json";
  summary.qualityGatesAlertGatePath = "/api/plans/quality-gates.alert.json?failOnFailed=true";
  summary.qualityGatesHealthPath = "/api/plans/quality-gates.health";
  summary.qualityGatesRemediationPath = "/api/plans/quality-gates.remediation.md";
  summary.qualityGatesRemediationGatePath = "/api/plans/quality-gates.remediation.md?failOnFailed=true";
  summary.qualityGatesBadgePath = "/api/plans/quality-gates.badge.json";
  summary.qualityGatesBadgeSvgPath = "/api/plans/quality-gates.badge.svg";
  summary.qualityGatesBadgeMarkdownPath = "/api/plans/quality-gates.badge.md";
  summary.qualityGatesJunitPath = "/api/plans/quality-gates.junit.xml";
  summary.qualityGatesJunitGatePath = "/api/plans/quality-gates.junit.xml?failOnFailed=true";
  summary.qualityGatesSarifPath = "/api/plans/quality-gates.sarif.json";
  summary.qualityGatesSarifGatePath = "/api/plans/quality-gates.sarif.json?failOnFailed=true";
  summary.qualityGatesStepSummaryPath = "/api/plans/quality-gates.step-summary.md";
  summary.qualityGatesStepSummaryGatePath = "/api/plans/quality-gates.step-summary.md?failOnFailed=true";
  summary.qualityGatesAnnotationsPath = "/api/plans/quality-gates.annotations.txt";
  summary.qualityGatesAnnotationsGatePath = "/api/plans/quality-gates.annotations.txt?failOnFailed=true";
  summary.qualityGatesOutputsPath = "/api/plans/quality-gates.outputs.txt";
  summary.qualityGatesOutputsGatePath = "/api/plans/quality-gates.outputs.txt?failOnFailed=true";
  summary.qualityGatesPrCommentPath = "/api/plans/quality-gates.pr-comment.md";
  summary.qualityGatesPrCommentGatePath = "/api/plans/quality-gates.pr-comment.md?failOnFailed=true";
  summary.qualityGatesArtifactsPath = "/api/plans/quality-gates.artifacts.json";
  summary.qualityGatesArtifactsGatePath = "/api/plans/quality-gates.artifacts.json?failOnFailed=true";
  summary.qualityGatesCommandsPath = "/api/plans/quality-gates.commands";
  summary.qualityGatesCommandsTextPath = "/api/plans/quality-gates.commands.txt";
  summary.qualityGatesCiGuidePath = "/api/plans/quality-gates.ci.md";
  summary.qualityGatesCiGuideGatePath = "/api/plans/quality-gates.ci.md?failOnFailed=true";
  summary.qualityGatesGatePath = "/api/plans/quality-gates?failOnFailed=true";
  summary.qualityGatesGateTextPath = "/api/plans/quality-gates.txt?failOnFailed=true";
  summary.qualityGatesCommandsGatePath = "/api/plans/quality-gates.commands?failOnFailed=true";
  summary.qualityGatesCommandsGateTextPath = "/api/plans/quality-gates.commands.txt?failOnFailed=true";
  summary.qualityGatesGateCommands = buildQualityGateCommands();
  const qualityGatesGateCommandByKey = Object.fromEntries(summary.qualityGatesGateCommands.map((command) => [command.key, command.command]));
  summary.qualityGatesGateCurlCommand = qualityGatesGateCommandByKey["curl-text"] || "";
  summary.qualityGatesGateJsonCurlCommand = qualityGatesGateCommandByKey["curl-json"] || "";
  summary.qualityGatesCsvGateCurlCommand = qualityGatesGateCommandByKey["curl-csv"] || "";
  summary.qualityGatesReportGateCurlCommand = qualityGatesGateCommandByKey["curl-report"] || "";
  summary.qualityGatesMetricsGateCurlCommand = qualityGatesGateCommandByKey["curl-metrics"] || "";
  summary.qualityGatesEventsGateCurlCommand = qualityGatesGateCommandByKey["curl-events"] || "";
  summary.qualityGatesAlertGateCurlCommand = qualityGatesGateCommandByKey["curl-alert"] || "";
  summary.qualityGatesHealthCurlCommand = qualityGatesGateCommandByKey["curl-health"] || "";
  summary.qualityGatesRemediationGateCurlCommand = qualityGatesGateCommandByKey["curl-remediation"] || "";
  summary.qualityGatesGateNpmCommand = qualityGatesGateCommandByKey["npm-text"] || "";
  summary.qualityGatesGateJsonNpmCommand = qualityGatesGateCommandByKey["npm-json"] || "";
  summary.qualityGatesCsvGateNpmCommand = qualityGatesGateCommandByKey["npm-csv"] || "";
  summary.qualityGatesReportGateNpmCommand = qualityGatesGateCommandByKey["npm-report"] || "";
  summary.qualityGatesMetricsGateNpmCommand = qualityGatesGateCommandByKey["npm-metrics"] || "";
  summary.qualityGatesEventsGateNpmCommand = qualityGatesGateCommandByKey["npm-events"] || "";
  summary.qualityGatesAlertGateNpmCommand = qualityGatesGateCommandByKey["npm-alert"] || "";
  summary.qualityGatesHealthNpmCommand = qualityGatesGateCommandByKey["npm-health"] || "";
  summary.qualityGatesRemediationGateNpmCommand = qualityGatesGateCommandByKey["npm-remediation"] || "";
  summary.qualityGatesCommandsGateCurlCommand = qualityGatesGateCommandByKey["curl-commands-text"] || "";
  summary.qualityGatesCommandsGateJsonCurlCommand = qualityGatesGateCommandByKey["curl-commands-json"] || "";
  summary.qualityGatesCiGuideGateCurlCommand = qualityGatesGateCommandByKey["curl-ci-guide"] || "";
  summary.qualityGatesJunitGateCurlCommand = qualityGatesGateCommandByKey["curl-junit"] || "";
  summary.qualityGatesSarifGateCurlCommand = qualityGatesGateCommandByKey["curl-sarif"] || "";
  summary.qualityGatesStepSummaryGateCurlCommand = qualityGatesGateCommandByKey["curl-step-summary"] || "";
  summary.qualityGatesAnnotationsGateCurlCommand = qualityGatesGateCommandByKey["curl-annotations"] || "";
  summary.qualityGatesOutputsGateCurlCommand = qualityGatesGateCommandByKey["curl-outputs"] || "";
  summary.qualityGatesPrCommentGateCurlCommand = qualityGatesGateCommandByKey["curl-pr-comment"] || "";
  summary.qualityGatesArtifactsGateCurlCommand = qualityGatesGateCommandByKey["curl-artifacts"] || "";
  summary.qualityGatesCommandsGateNpmCommand = qualityGatesGateCommandByKey["npm-commands-text"] || "";
  summary.qualityGatesCommandsGateJsonNpmCommand = qualityGatesGateCommandByKey["npm-commands-json"] || "";
  summary.qualityGatesCiGuideGateNpmCommand = qualityGatesGateCommandByKey["npm-ci-guide"] || "";
  summary.qualityGatesJunitGateNpmCommand = qualityGatesGateCommandByKey["npm-junit"] || "";
  summary.qualityGatesSarifGateNpmCommand = qualityGatesGateCommandByKey["npm-sarif"] || "";
  summary.qualityGatesStepSummaryGateNpmCommand = qualityGatesGateCommandByKey["npm-step-summary"] || "";
  summary.qualityGatesAnnotationsGateNpmCommand = qualityGatesGateCommandByKey["npm-annotations"] || "";
  summary.qualityGatesOutputsGateNpmCommand = qualityGatesGateCommandByKey["npm-outputs"] || "";
  summary.qualityGatesPrCommentGateNpmCommand = qualityGatesGateCommandByKey["npm-pr-comment"] || "";
  summary.qualityGatesArtifactsGateNpmCommand = qualityGatesGateCommandByKey["npm-artifacts"] || "";
  summary.qualityGatesCommandsGateCommandBundle = [
    summary.qualityGatesCommandsGateCurlCommand,
    summary.qualityGatesCommandsGateJsonCurlCommand,
    summary.qualityGatesCommandsGateNpmCommand,
    summary.qualityGatesCommandsGateJsonNpmCommand,
  ].filter(Boolean).join("\n");
  summary.qualityGatesGateCommandBundle = summary.qualityGatesGateCommands.map((command) => command.command).filter(Boolean).join("\n");
  summary.qualityGatesGateCiExamples = buildQualityGateCiExamples({ commands: summary.qualityGatesGateCommands });
  const qualityGatesGateCiExampleByKey = Object.fromEntries(summary.qualityGatesGateCiExamples.map((example) => [example.key, example]));
  summary.qualityGatesGateLocalShellPath = qualityGatesGateCiExampleByKey["local-shell"]?.path || "";
  summary.qualityGatesGateGithubActionsPath = qualityGatesGateCiExampleByKey["github-actions-node"]?.path || "";
  summary.qualityGatesGateGithubActionsExample = summary.qualityGatesGateCiExamples.find((example) => example.key === "github-actions-node")?.body || "";
  summary.qualityGatesGateLocalShellExample = summary.qualityGatesGateCiExamples.find((example) => example.key === "local-shell")?.body || "";
  summary.qualityGatePath = "/api/plans/quality-gate";
  summary.qualityGateTextPath = "/api/plans/quality-gate.txt";
  summary.qualitySoftGatePath = "/api/plans/quality-gate?maxActions=5";
  summary.qualitySoftGateTextPath = "/api/plans/quality-gate.txt?maxActions=5";
  summary.qualityUrgentGatePath = "/api/plans/quality-gate?urgent=true";
  summary.qualityUrgentGateTextPath = "/api/plans/quality-gate.txt?urgent=true";
  summary.qualityUrgentSoftGatePath = "/api/plans/quality-gate?urgent=true&maxActions=5";
  summary.qualityUrgentSoftGateTextPath = "/api/plans/quality-gate.txt?urgent=true&maxActions=5";
  summary.qualityNextGatePath = summary.qualityNextFilter
    ? "/api/plans/quality-gate?next=true"
    : "";
  summary.qualityNextGateTextPath = summary.qualityNextFilter
    ? "/api/plans/quality-gate.txt?next=true"
    : "";
  summary.qualityNextSoftGatePath = summary.qualityNextFilter
    ? "/api/plans/quality-gate?next=true&maxActions=5"
    : "";
  summary.qualityNextSoftGateTextPath = summary.qualityNextFilter
    ? "/api/plans/quality-gate.txt?next=true&maxActions=5"
    : "";
  summary.topQualityWarnings = [...warningCounts.entries()]
    .sort(([labelA, countA], [labelB, countB]) => countB - countA || labelA.localeCompare(labelB, "ko"))
    .slice(0, 3)
