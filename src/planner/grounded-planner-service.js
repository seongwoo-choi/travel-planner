...
async function resolveTripTimezone(collector, trip, now) {
  ...
  try {
    ...
    if (resolved?.status === "unavailable") {
      throw new TypeError(resolved.error || "timezone evidence is unavailable");
    }
    ...
    return {
      ...
      status: resolved.status === "degraded" ? "degraded" : "verified",
      ...(resolved.error ? { lookupError: resolved.error } : {}),
    };
  } catch (error) {
    return {
      timezone: null,
...
export async function createGroundedTripPlan({ trip, collectors, now = () => new Date() }) {
  ...
  const verifiedPlaces = placeSnapshot.status === "unavailable"
    ? []
    : placeSnapshot.items.filter(hasVerifiedOpeningHours);
  ...
  const plan = planTrip({
    ...
    travelMinutes: travelSnapshot.status === "unavailable" ? {} : travelSnapshot.matrix,
  });
  const unavailablePlaceIds = placeSnapshot.status === "unavailable"
    ? placeSnapshot.items.map((place) => place.id)
    : [];
  plan.unscheduledPlaceIds = [
    ...new Set([...plan.unscheduledPlaceIds, ...unverifiedPlaces.map((place) => place.id), ...unavailablePlaceIds]),
  ];
  const placesById = new Map(placeSnapshot.items.map((place) => [place.id, place]));
  for (const [name, snapshot] of [["PLACES", placeSnapshot], ["TRAVEL", travelSnapshot]]) {
    if (snapshot.status !== "unavailable" && !(name === "PLACES" && snapshot.status === "degraded")) continue;
    plan.verificationTasks.push({
      code: `${name}_EVIDENCE_${snapshot.status.toUpperCase()}`,
      message: `${name.toLowerCase()} evidence is ${snapshot.status} and must be confirmed`,
    });
  }
  ...
  if (weatherSnapshot.status === "degraded"
    && !plan.verificationTasks.some((task) => task.code.startsWith("WEATHER_"))) {
    plan.verificationTasks.push({
      code: "WEATHER_EVIDENCE_DEGRADED",
      message: "weather evidence is degraded and must be confirmed",
    });
  }
  ...
  if (!timezone) {
    ...
    plan.verificationTasks.push({
      ...
      timezone,
      message: timezoneSnapshot.lookupError
        ? `Independent timezone lookup failed (${timezoneSnapshot.lookupError}), so ${timezone} from ${timezoneSnapshot.source} was used for time-dependent transit departure timing and must be confirmed`
        : `Timezone evidence from ${timezoneSnapshot.source} is degraded, so ${timezone} must be confirmed`,
    });
  }
  ...
  if (travelSnapshot.status === "degraded"
    && !plan.verificationTasks.some((task) => task.code === "MAJOR_TRANSPORT_UNAVAILABLE")) {
    plan.verificationTasks.push({
      code: "TRAVEL_EVIDENCE_DEGRADED",
      message: "travel evidence is degraded and must be confirmed",
    });
  }
